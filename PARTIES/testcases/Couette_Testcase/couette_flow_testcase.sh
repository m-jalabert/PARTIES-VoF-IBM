...
echo 'Starting PARTIES test run: Couette flow'
echo 'Version 1.0'
...
for i in `grep -n PERIODIC_SHEAR_FLOW Boundary.h | gawk '{print $1}' FS=":"`
do
    let "count+=1"
    if [ $count -eq 1 ]
    then
...
echo '      U(y)' > numerical_velo_CF.dat
awk "NR==$((header_line_no+1)), NR==$((header_line_no+100))" velo.dat | gawk '{print $2}' FS=": " | gawk '{print $1}' FS="," >> numerical_velo_CF.dat

###########################################################################
#                              Comparision                                #
###########################################################################
# Gives numerical minus analytical: Absolute value
# FNR here relates to the corresponding gird number of the control volume 
# awk 'FNR==NR { _a[FNR]=$1;} NR!=FNR { $1 -= _a[FNR]; {sub("^-", "", $1); if ($1 > 0.02) print $1,"FNR="FNR};  }'  analytical_velo_CF.dat numerical_velo_CF.dat

# L2 Norm
g++ l2norm_vector.c -o l2norm.sh
./l2norm.sh
