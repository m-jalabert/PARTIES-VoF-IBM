#!/bin/bash
# Shell script to run a test for Volume Imposed Testcase with the PARTIES fluid solver.
# v1

# Current directory will be /testcases/Rheology_Testcase/Volume_Imposed_Testcase
echo '----------**Start Volume Imposed Testcase**----------'
home_path=`pwd`
...
###########################################################################
cd ../../
mkdir Volume_Imposed_run
chmod 777 Volume_Imposed_run
cp $home_path/*.* Volume_Imposed_run/
cd Volume_Imposed_run
work_path=`pwd`

###########################################################################
#                        Setting the Boundary.h                           #
###########################################################################
cp Boundary_VI.h ../../src/Include
cd ../../src/Include
boundary_path=`pwd`
mv Boundary.h Boundary_ORIG.h
mv Boundary_VI.h Boundary.h
...
printf 'make clean\nmake\n'
make clean > make_Volume_Imposed.log
make >> make_Volume_Imposed.log 2>&1
echo 'END: Compiling executable'

cp parties $work_path
mv make_Volume_Imposed.log $work_path
cd $boundary_path
rm -rf Boundary.h
...
#                                L2 Norm                                  #
###########################################################################
mv numerical_velo_verified_VI.dat numerical_velo_verified.dat
...
then
    ...
    mv numerical_velo.dat numerical_velo_verified_VI.dat
    mv absolute_error.dat absolute_error_VI.dat
    # Clean up
    rm -rf velo.dat
    cd $home_path/../../
    (exit 0)
    ...
    mv numerical_velo_verified.dat numerical_velo_verified_VI.dat
    mv numerical_velo.dat numerical_velo_failed_VI.dat
    mv absolute_error.dat absolute_error_failed_VI.dat
    # Clean up
    rm -rf velo.dat
    cd $home_path/../../
    (exit 1)
