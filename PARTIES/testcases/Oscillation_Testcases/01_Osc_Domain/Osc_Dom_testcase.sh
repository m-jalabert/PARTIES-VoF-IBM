...
echo '----------**Start Oscillation_Domain Testcase**----------'
home_path=`pwd`
...
cd ../../
mkdir Osc_Dom_run
chmod 777 Osc_Dom_run
cp $home_path/*.* Osc_Dom_run/
cd Osc_Dom_run
work_path=`pwd`
...
###########################################################################
cp Boundary_Osc_Dom.h ../../src/Include
cd ../../src/Include
boundary_path=`pwd`
mv Boundary.h Boundary_ORIG.h
mv Boundary_Osc_Dom.h Boundary.h
...
printf 'make clean\nmake\n'
make clean > make_Osc_Dom.log
make >> make_Osc_Dom.log 2>&1
echo 'END: Compiling executable'

cp parties $work_path
mv make_Osc_Dom.log $work_path
cd $boundary_path
rm -rf Boundary.h
