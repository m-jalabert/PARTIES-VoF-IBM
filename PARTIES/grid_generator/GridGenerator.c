...
// Slope of the nonuniform region if there is only one region. Other than that,
// the slope of the nonuniform region will be computed using the uniform region
#define END_SLOPE 4.6875
...
struct subregion {
	...
	// min and max of the sub region
	double lmin, lmax;
	
	// Number of grids in the current domain
	double N;
...
struct gridinfo {
	...
	// Max and minumum coordinate of the domain
	double Lmin, Lmax;
	
	// Total number of grid points
	double N;
	
	// array holding all the sub_region properties
	...
	// position of the side cell, (one more than the number of grids)
	double *position_s;
}; 
...
void GridGen_set_sub_region_type(GridInfo *grid, int *region_type) ;
...
double GridGen_compute_Q(double slope, int which_equation) ;
...
int main()  {
	...
	/*
	  ...
	  3 - NONUNIFORM_ONE_SIDED: non-uniform with the slope  defined at the
	      beginnning
	 
	 Try to have uniform grid bounding NONUNIFORM_TWO_SIDED sub-region.
	  */
	 /*-----------------------------------------------------------------------*/
//	int Region_type[] = {UNIFORM, NONUNIFORM_TWO_SIDED};
	int Region_type[] = {NONUNIFORM_TWO_SIDED};
//	int Region_type[] = {UNIFORM};
	...
	/*
	 ...
	 1 - First, grid side (e.g. x position of u-grid in a MAC-grid)
	     (N_total+1)
	 2 - Second, grid cell center (e.g. x position of center-grid in a MAC-grid)
	     (N_total)
	 */
	/*------------------------------------------------------------------------*/
...
/******************************************************************************/
/*
 This function sets the number of sub_regions in each dierction. Note that for
 uniform grid, we only have one region. For the non-uniform regions, one can set
 any arbitrary regions > 1
 */
/******************************************************************************/
void GridGen_set_sub_region_type(GridInfo *grid, int *region_type) {
...
void GridGen_compute_sub_region_coordinates(GridInfo *grid, int region_id) {
	...
	// Number of grid cells in the current sub region
	Nregion = grid->sub_region[region_id]->N;
	
	// uniform delta_e in the transfomred coordinates system
	dEta    = 1.0/(double) Nregion;
...
/******************************************************************************/
/*
 This function sets the stretching function based on the slope given at the
 boundary nodes
 */
/******************************************************************************/
void GridGen_set_stretching_function_type(SubRegion *sub_region) {
	...
	switch (sub_region->type) {
		...
		// If the nonuniform grid boundary condition is given at one side 
		//(probably only the left side)
...
/******************************************************************************/
/*
 This function returns the value of the real coordinate based on the given
 stretching function for the given subregion
 */
/******************************************************************************/
double GridGen_stretching_function_value(SubRegion *sub_region, double eta) {
...
/******************************************************************************/
/*
 This function sets the next sub_region's boundary properties based on the
 previous one
 */
/******************************************************************************/
void GridGen_sub_region_boundary_properties(GridInfo *grid, int region_id) {
	...
	if (region_id == 0) {
		...
		if (grid->sub_region[region_id]->type == UNIFORM) {
			...
			// This ensures that the slope at first grids of the two successive 
			...
			if ( (Nsub_region - region_id) > 1 ) {
				...
				if ( ( grid->sub_region[region_id]->type == NONUNIFORM_TWO_SIDED ) && (grid->sub_region[region_id+1]->type == UNIFORM) ) {
					...
					// For now, assume that the two sided nonuniform mesh is 
					// anti-symmetric
					S1 = S0;
				}
			}
	...
	// This ensures that the slope and first grids of the two successive 
	// regions are the same
	A = sqrt(S0 / S1);
	B = sqrt(S0 * S1);
...
/******************************************************************************/
/*
 This function writes the grid postion to file. First cell center coordinates
 and then the side-cell coordinates.
 */
/******************************************************************************/
void GridGen_write_grid(GridInfo *grid) {
