...
struct volume_fraction {
	...
	double ***rho_old;   // ρ^{k-1}: density at the previous RK stage
	double ***mu_old;    // μ^{k-1}: viscosity at the previous RK stage
	...
	char   ***mcl_mask;     // Ghost-band marker for the diffuse contact-angle model
	#endif
...
struct parameters {
	...
	double slip_length;		  // Slip length for the contact angle model
